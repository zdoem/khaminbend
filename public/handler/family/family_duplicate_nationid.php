...
if(isset($_POST['id'])&&strlen(trim(@$_POST['id']))>0){
    ...
    WHERE  mem_citizen_id=? AND mem_fam_id!=? AND RIGHT(YEAR(d_survey),2)  IN (
        SELECT RIGHT(YEAR(d_survey),2) FROM  fm_fam_hd  WHERE mem_fam_id=? 
    ...
    $rows_old =$db::select("SELECT mem_citizen_id FROM  fm_fam_hd AS a INNER JOIN fm_fam_members_dt1 AS b ON a.fam_id=b.mem_fam_id
    WHERE  mem_citizen_id=? AND RIGHT(YEAR(d_survey),2)  IN (
